...
********************************以下为官网例程的内容
*/
void IINCHIP_WRITE(uint32_t addrbsb,uint8_t data){//MCU将单字节数据写入指定地址
  w5500_CSOFF();
  SPI_SendByte( (addrbsb & 0x00FF0000)>>16);	// 地址段1
  SPI_SendByte( (addrbsb & 0x0000FF00)>> 8);	// 地址段2
  SPI_SendByte( (addrbsb & 0x000000F8) + 4);  // 控制段
  SPI_SendByte(data); 
  w5500_CSON();
}
uint8_t IINCHIP_READ(uint32_t addrbsb){//MCU向W5500读取单字节数据
  w5500_CSOFF();
  SPI_SendByte( (addrbsb & 0x00FF0000)>>16);	// 地址段1
  SPI_SendByte( (addrbsb & 0x0000FF00)>> 8);	// 地址段2
  SPI_SendByte( (addrbsb & 0x000000F8));  // 控制段
  uint8_t data = SPI_ReadByte();
  w5500_CSON();
  return data;
}
uint16_t wiz_write_buf(uint32_t addrbsb,uint8_t* buf,uint16_t len){//向W5500发送写指令
  w5500_CSOFF();
  uint16_t index = 0;//发送位数计数
  SPI_SendByte((addrbsb & 0x00FF0000)>>16 );//地址段1
  SPI_SendByte( (addrbsb & 0x0000FF00)>> 8 );//地址段2数据
  SPI_SendByte( (addrbsb & 0x000000F8) + 4 );//控制段数据，+4（0000 0100 RWB位置上1表示写入，OM位为00表示SPI工作模式为VDM）
...
uint16_t wiz_read_buf(uint32_t addrbsb, uint8_t* buf,uint16_t len){//向W5500发送读指令
  w5500_CSOFF();
  uint16_t index = 0;//发送位数计数
  SPI_SendByte((addrbsb & 0x00FF0000)>>16 );//取地址
  SPI_SendByte( (addrbsb & 0x0000FF00)>> 8 );//控制段1数据
  SPI_SendByte( (addrbsb & 0x000000F8));//控制段2数据，+4（0000 0000 RWB位置上0表示读取，OM位为00表示SPI工作模式为VDM）
  for(index = 0; index < len; index++){
    buf[index] = SPI_ReadByte();
  }
  w5500_CSON();
  return len;
}
